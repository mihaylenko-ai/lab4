...
- eBGP - протокол динамической маршрутизации относящийся к классу протоколов маршрутизации внешнего шлюза.
- Автономная система (AS) - сеть из одного или несколькоих маршрутизаторов, управляемых одним или несколькими операторами, имеющие единую политику маршрутизации.
...
устройства в одну автономую систему (AS65500), у то время как каждое устройство Leaf будет находится в своей автономной системе (AS65501-AS65503). В силу того, что устройств Spine
...
Здесь следует уточнить, что в данной лабораторной работе адресное пространство ip адресов служебных каналов выбрано не самое оптимальное, и для описания listen reange придется использовать
маску 255.0.0.0. Однако в силу унификации лабораторных работ и последующего их анализа остаим подобную адресацию.
...
В цачестве описания настроек будем использовать группы параметров. Типовые настройки каждого устройства автономной системы сведем к уменьшению таймеров исходя из соотношения
1:3 keepalive к hold, что ускорит время реакции на нештатные ситуации с каналом. Кроме того исправим external-distance, internal-distance и local-distance. В качестве настроек
соседей зададим мгновенное время реакции на изменение топологии, а так же включим протокол BFD, для более точного отследивания проблем на кагалах связи между утсройствами.
В качестве шифрования для примера будем использовать единый пароль "cisco".

Следует отметить, что для обеспечения устройствами Leaf обеих маршрутов до устройств Spine включим поддержку ECMP для двух маршрутов.
